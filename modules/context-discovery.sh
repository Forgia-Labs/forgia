...
discover_context_formatted() {
  ...
  _cd_format_output "$format" "$sorted"
}
...
USAGE
  ...
  # Add --include paths to the shared results array
  ...
  # Delegate to discover_context_formatted which handles sorting, limits, and output
  # Skip its _cd_discover call since we already ran it above with --include paths
  if [[ ${#_cd_results[@]} -eq 0 ]]; then
    return 1
  fi
